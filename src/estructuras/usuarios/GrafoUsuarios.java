...
public class GrafoUsuarios {
    ...
    private boolean[][] matriz;        // matriz de adyacencia, utilizamos el tipo bool para documentar true si se sigue pero false no se sigue.
    ...
    public boolean agregarUsuario(Usuario usuario) {
        if (cantidad >= capacidad) return false;    //se maneja la logica de agregar usuario, y tambien controlamos la cantidad que sea menor o igual a la capacidad
    ...
    private int obtenerIndice(int idUsuario) {       //usamos este metodo para obtener el id del usuario, si es que este se encuentra en el array de usuarios,
                                                        ///retornamos -1 si no se enncuentra el usuario.
        for (int i = 0; i < cantidad; i++) {
            if (usuarios[i].getId() == idUsuario) return i;
        }
        return -1;
    }
    ...
    public boolean sigueA(int idA, int idB) {  // para identificar si i segue a j, si el valor de la posicion es true, i lo sigue de lo contrario es false
    ...
    public Usuario[] obtenerSeguidos(int idUsuario) {   
        ...
        if (i == -1)
            return new Usuario[0]; //si no existe el usuario con ese id, retornamos el array vacio dado que no tendria relaciones

        int contador = 0;
        for (int j = 0; j < cantidad; j++) {     ///este for analizamos las relaciones del usuario en la matriz, contamos cuantas relaciones tiene el usuario es decir,
            if (matriz[i][j])                    //en los casso que sea true. Para posteriormente usar ese contador ppara designar el largo del array
                contador++;
        }
    ...
    public Usuario[] obtenerSeguidores(int idUsuario) {   ///la logica es igual a obtenerseguidos, pero en este caso buscariamos en los otros usuarios que quienes 
